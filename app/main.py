...
async def debug_graph_status():
    """Debug endpoint to see actual graph state"""
    if not rag_system or not hasattr(rag_system, 'neo4j'):
    ...
    try:
        with rag_system.neo4j.driver.session() as session:
            ...
            # List documents
            doc_result = session.run("MATCH (d:Document) RETURN d.title as title LIMIT 20")
            documents = [record["title"] for record in doc_result]
...
async def clear_documents():
    """Clear all documents with explicit transaction commit"""
    if not rag_system:
        raise HTTPException(status_code=503, detail="GraphRAG system not available")
    ...
    try:
        if hasattr(rag_system, 'neo4j') and rag_system.neo4j.driver:
            with rag_system.neo4j.driver.session() as session:
                ...
                print(f"Clearing {doc_count} documents from Neo4j...")
                ...
                if remaining > 0:
                    print(f"WARNING: {remaining} nodes still remain after delete")
                else:
                    print("All Neo4j nodes successfully deleted")
                ...
                if postgres and postgres.conn:
                    try:
                        with postgres.conn.cursor() as cursor:
                            ...
                            postgres.conn.commit()
                        print("PostgreSQL documents cleared")
                    except Exception as e:
                        print(f"PostgreSQL clear warning: {e}")
                
                # Clear uploaded files
                files_removed = 0
                for file_path in list(file_storage.values()):
                    if os.path.exists(file_path):
                        try:
                            os.remove(file_path)
                            files_removed += 1
                ...
                file_storage.clear()
                print(f"Removed {files_removed} uploaded files")
                
                # Clear embedding cache in RAG system
                if hasattr(rag_system, 'embedding_cache'):
                    cache_size = len(rag_system.embedding_cache)
                    rag_system.embedding_cache.clear()
                    print(f"Cleared {cache_size} cached embeddings")
                
                return {
                    "message": f"Cleared {doc_count} documents successfully",
